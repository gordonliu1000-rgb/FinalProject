...
void Mob::update(){
    ...
    if(bitmap_switch_counter > 0) //一張圖要持續多久
        ...
        can_switch_state_bitmap = (bitmap_img_id == 0);
    }
    ...
    switch(state){
        ...
        case MobState::ATK:{
            if(atk_cool_down==0 && bitmap_img_id==attack_frame_id){
                atk_hero();
            }

            if(can_switch_state_bitmap){//攻擊完後才能切成閒置
                state = MobState::IDLE;
                bitmap_img_id = 0;
            }
            break;
        ...
        case MobState::HURT:{
            if(bitmap_img_id==(bitmap_img_ids[static_cast<int>(MobState::HURT)][static_cast<int>(dir)]-1))
                state = MobState::WALK;
            break;
        }
        case MobState::DIE:{
...
void Mob::draw(){
    ImageCenter *IC = ImageCenter::get_instance();
    static MobState prev_state = MobState::WALK;
    char buffer[50];
    if(state==MobState::IDLE){
        sprintf(
		buffer, "%s/%s/%s/%d.png",
		MobSetting::mob_imgs_root_path[static_cast<int>(type)],
        MobSetting::state_path_prefix[static_cast<int>(prev_state)],
		MobSetting::dir_path_prefix[static_cast<int>(dir)],
		bitmap_img_id);
    }
    else{
